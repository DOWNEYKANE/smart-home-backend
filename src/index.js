...
// 清除模拟数据（免登录，GET方式浏览器直接打开）
app.get('/api/admin/clear-simulator', (req, res) => {
  const { getPool } = require('./db')
  const pool = getPool()
  pool.execute('DELETE FROM device_data')
  res.json({ code: 0, message: '数据已清空' })
})
...
async function start() {
  try {
    ...
    // 机智云 API 拉温湿度组数据
    const { connectGizwits } = require('./gizwits')
    connectGizwits()
