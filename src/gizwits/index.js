...
function fetchDeviceData() {
  ...
  const options = {
    hostname: 'api.gizwits.com',
    path: '/app/devdata/' + PRODUCT_KEY + '/' + DEVICE_MAC + '/latest',
    method: 'GET',
    headers: {
      'X-Gizwits-Application-Id': APP_ID,
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      try {
        const r = JSON.parse(body)
        if (r.attr || r.body) {
          const attrs = r.attr || r.body || {}
          // 机智云属性映射
          const data = {
            temperature: attrs.temperature || attrs.Temperature || 0,
            humidity: attrs.humidity || attrs.Humidity || 0,
            smoke: attrs.smoke || attrs.Smoke || 0,
            relayStatus: attrs.relay_switch ? 'on' : 'off',
            deviceStatus: (r.is_online || r.online) ? 'online' : 'offline',
