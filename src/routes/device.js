...
router.post('/:deviceType/control', authRequired, async (req, res) => {
  try {
    ...
    // 浇灌组走 OneNET MQTT 下发指令
    if (deviceType === 'irrigation') {
      try {
        const { sendCommand } = require('../onenet/command')
        await sendCommand(command)
        return res.json({ code: 0, message: '指令已发送到OneNET', command })
      } catch (e) {
        return res.json({ code: 0, message: '指令已记录（OneNET MQTT未连接）', command })
      }
    }
...
async function sendOneNETCommand(command) {
  ...
  return new Promise((resolve, reject) => {
    const options = {
      ...
      path: '/device/thing/property/set?product_id=9H5ymnKKrW&device_name=T1',
      method: 'POST',
      headers: {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        console.log('[OneNET] 属性设置 HTTP' + res.statusCode + ':', data.substring(0, 300))
        try {
          if (data.trim()) {
            const r = JSON.parse(data)
            if (r.code === 0) resolve(r)
            else reject(new Error(r.msg || '非0返回'))
          } else {
