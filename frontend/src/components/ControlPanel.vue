    ...
    <div class="control-grid">
      <div class="control-item" v-for="ctrl in controls" :key="ctrl.key">
        ...
        <div v-else-if="ctrl.type === 'threshold'" class="threshold-group">
          <input type="number" v-model="thresholdValues[ctrl.key]" />
          <span class="threshold-unit">{{ ctrl.unit }}</span>
          <button class="btn-set" @click="emit('command', { action: ctrl.key, value: thresholdValues[ctrl.key] })">设置</button>
        </div>
...
watch(() => props.controls, (newControls) => {
  newControls.forEach(c => {
    if (c.type === 'threshold' && !(c.key in thresholdValues.value)) {
      thresholdValues.value[c.key] = c.value
    }
  })
