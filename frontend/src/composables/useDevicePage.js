...
export function useDevicePage(deviceType, fetchFn, historyApi, historyParams = {}) {
  ...
  const chineseLabel = {
    temperature: '温度(°C)', humidity: '湿度(%)', smoke: '烟雾(ppm)',
    spo2: '血氧(%)', heartRate: '心率(bpm)',
    soilMoisture: '土壤湿度(%)', foodRemaining: '粮食剩余(g)'
  }
  ...
  onMounted(() => {
    loadLatest()
    loadHistory()
    timer = setInterval(() => { loadLatest(); loadHistory() }, 1000)
  })
